...
class Monticulo:
    ...
    ##corregido
    #def __init__(self, key=None):
    #    # Inicializa el montículo con una lista que comienza en índice 1
    #    self.lista= [None]
    #    self.tamanoActual = 0
    #    #key: función o atributo para comparar elementos
    #    self.key = key if (callable(key) or isinstance(key, str)) else (lambda x: x)
    #sin key
    def __init__(self):
        # índice 0 sin usar para facilitar cálculos padre/hijo
        self.lista = [None]
        self.tamanoActual = 0
    ...
    def _infiltArriba(self, i):
        while i // 2 > 0:
            if self.lista[i] < self.lista[i // 2]:  # cambiar _menor por <
                self.lista[i], self.lista[i // 2] = self.lista[i // 2], self.lista[i]
            i //= 2
    ...
    def eliminarminimo(self):
        if self.estavacio():
            return None
        minimo = self.lista[1]
         # mover último a la raíz y quitar último
    ...
    def _infiltAbajo(self, i):
        while (i * 2) <= self.tamanoActual:
            hijo = i * 2
            # elegir el hijo menor
            if hijo + 1 <= self.tamanoActual and self.lista[hijo + 1] < self.lista[hijo]:
                hijo += 1
    ...
    def tamano(self):
        # método solicitado para obtener el tamaño
        return self.tamanoActual

    def __len__(self):
        # permite usar len(monticulo)
        return self.tamanoActual
    ...
    def construirMonticulo(self, unaLista):
        """Construye montículo desde una lista (no incluir None inicial)."""
        self.lista = [None] + list(unaLista)
        self.tamanoActual = len(unaLista)
    ...
    def __contains__(self, elemento):
        ...
        return elemento in self.lista[1:self.tamanoActual + 1]
        
    def decrementar_clave(self, elemento_viejo, elemento_nuevo):
    ...
    # Busco la posición (índice) del elemento_viejo en la lista interna (saltando índice 0)
        ...
        for i in range(1, self.tamanoActual + 1):
            if self.lista[i] == elemento_viejo:
                indice = i
                break
        ...
        while indice > 1 and self.lista[indice] < self.lista[indice // 2]:
            # Intercambio con el padre
            self.lista[indice], self.lista[indice // 2] = self.lista[indice // 2], self.lista[indice]
